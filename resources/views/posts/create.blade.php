...
<div class="p-4 rounded-xl shadow-xl border-2 border-black w-3/4 mx-auto">
    <form action="{{route('posts.store')}}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" value="{{@old('titulo')}}" id="titulo" name="titulo" class="w-full p-3 border border-gray-300 rounded-lg" placeholder="Escribe el título">
            @error('titulo')
            <x-error>
                {{$message}}
            </x-error>
        ...
        <div class="mb-4">
            ...
            <textarea id="contenido" name="contenido" class="w-full p-3 border border-gray-300 rounded-lg" rows="4" placeholder="Escribe el contenido aquí">{{@old('contenido')}}</textarea>
            @error('contenido')
            <x-error>
                {{$message}}
            </x-error>
        ...
        <div class="mb-4">
            ...
            <div class="flex items-center space-x-4">
                <label class="inline-flex items-center">
                    <input type="radio" @checked(@old('categoria')=='Publicado') 
                    name="categoria" value="Publicado" class="form-radio text-blue-500">
                    <span class="ml-2">Publicado</span>
                </label>
                <label class="inline-flex items-center">
                    <input type="radio" @checked(@old('categoria')=='Borrador')  
        ...
        <div class="flex items-center justify-between">
            <button type="reset" class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 focus:outline-none">
